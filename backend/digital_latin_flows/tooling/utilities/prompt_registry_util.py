...
# user, and judge prompt templates, mapping prompt IDs to their corresponding
# Jinja2 template filenames. The registries are built at import time from
# version definitions ensuring that all prompts are available for lookup.
# -------------------------------------------------------------

# Version definitions for each prompt type

# The key, level, and version are used to uniquely identify the prompt and map to elements of the filename
# Example
...
# System prompt file pattern
def system_prompt_filename(system_key, level, version):
    """Return the filename for a system prompt template."""
...
def user_prompt_filename(system_key, author):
    """Return the filename for a user prompt template."""
...
# Utility methods used by the prompt selector tool to retrieve prompt filenames
# when only the key (i.e. S1.2A) is provided
def get_system_prompt(key):
    """Lookup a system prompt filename by key, with validation."""
...
def get_user_prompt(key):
    """Lookup a user prompt filename by key, with validation."""
    lookup_key = key[0].lower() + key[1:]
    try:
...
def get_judge_prompt(key, prompt_type="system"):
    """Lookup a judge prompt filename by key and type, with validation."""
    lookup_key = key[0].lower() + key[1:]
