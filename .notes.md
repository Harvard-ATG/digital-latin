...
- [X] Add custom CSS for section title and bullet spacing in assistant messages
- [X] Use environment variable for Streamlit port in entrypoint.sh
- [X] Review and document all required environment variables for production
...
## Deployment Refactor & Local Development Notes (Terraform/ECS Readiness)

- After moving the Streamlit code into `/frontend`, the Dockerfile was refactored to match the new directory structure, ensuring all necessary files are copied and installed from the correct locations.
- For local development, use `docker-compose.local.yml` instead of `docker-compose.yml`. The local file includes a containerized PostgreSQL database, which is not present in the default compose file.
- The PostgreSQL container requires `POSTGRES_PASSWORD` to be set for successful startup and connections. Setting `POSTGRES_AUTHENTICATION_METHOD=no` disables password authentication, but this is not recommended for security reasons.
- Added `depends_on` in the Streamlit service configuration to ensure the PostgreSQL container starts before Streamlit. Note: `depends_on` only waits for the container to start, not for the database to be ready to accept connections.
- To address this, a `pg_isready` check and sleep loop were added in `entrypoint.sh` to wait for PostgreSQL to be ready before starting Streamlit. The PostgreSQL client (`pg_isready`) is installed in the container via the Dockerfile before running the entrypoint script.
- All sensitive and configuration variables (DB credentials, API keys, etc.) are managed via `.env` files and referenced in the compose files. `.env` is included in `.gitignore` to prevent accidental commits of secrets.
- The `volumes` section in docker-compose mounts the local `app` directory into the container, allowing live code updates without rebuilding the image.
- The entrypoint script handles environment debugging, waits for database readiness, and starts the Streamlit app, ensuring robust startup and easier troubleshooting.
- Always rebuild the image after Dockerfile or dependency changes (`docker-compose build --no-cache`).
- Use `.local.yml` for local overrides and development-specific settings.
- Keep production and local configurations separate for security and reliability.
- To remove all containers and volumes (useful for a clean local reset):
  ```bash
  docker-compose down -v
  ```
- Example command:
  ```bash
  docker compose -f 'frontend/docker-compose.local.yml' down -v
  ```
- These changes and practices ensure the codebase is ready for deployment to Terraform-managed AWS ECS environments, with clear separation between local development and production deployment workflows.

## Authentication Approaches

There are several possible authentication paths for the Digital Latin Streamlit frontend:

- **Current Approach:**
  - Because the number of users is very small and the timeline is tight, we opted for a simple shared login: a small list of usernames and passwords (managed via environment variables) that can be distributed to authorized users. This is implemented with a login form at the top of the Streamlit app, and credentials are checked against the environment variables.

- **Alternative: Streamlit Built-in Authentication with Database Integration:**
  - Streamlit offers authentication libraries that can be integrated with a user schema in the existing application database. This would allow for more robust user management, including registration, password reset, and user roles.
  - An admin page or section could be created (visible only to admin users) to approve or manage new user accounts.

- **OIDC, Harvard Key Integration:**
  - For broader or long-term use, Harvard Key, Google OIDC, (or another institutional SSO) could be integrated for authentication. This was not chosen for the initial launch due to the small user base and short timeline, but remains an option for future expansion.

- **Summary:**
  - The current approach is simple and effective for a small, trusted user group.
  - As the user base grows or security needs increase, the authentication system can be upgraded to use database-backed user management or institutional SSO.

## Session Management and Single-Session Enforcement

- With the current simple authentication (shared credentials), multiple users can log in simultaneously with the same username and password, as there is no session tracking or lockout logic. This is acceptable for a small, trusted user group but is not secure or auditable for larger deployments.
- **Database-Backed Session Management:**
  - Each login creates a session record in the database, with fields like user_id, session_id, status (active/inactive), login/logout timestamps, etc.
  - This allows tracking of who is logged in, when, and from where, and enables enforcement of single-session-per-user (if desired): before allowing a login, check if the user has an active session.
  - Session data (e.g., chat history, preferences) can be associated with the user.
  - Forced logouts, session expiration, and audit trails can be implemented.
- **In-Memory or Cookie-Based Session Management:**
  - Use Streamlit’s session state, a server-side cache (like Redis), or browser cookies to track sessions. This is easier for small apps but harder to enforce single-session-per-user across devices/browsers.
- **Hybrid Approach:**
  - Use the database for persistent user/session tracking, and in-memory/cookie for fast access and UI state.
- **Enforcing Single Session per User:**
  - On login, check the session table for an active session for that user. If found, deny login or prompt to terminate the previous session. On logout (or after a timeout), mark the session as inactive.
- **Recommendation:**
  - For a small, trusted user group, the current approach is fine. For more users or higher security, add a user table and another session table to the database, track session status, and enforce single-session-per-user if needed.

## Handling Abandoned Sessions

- If a user closes the browser or loses connection without logging out, their session may remain marked as "active" in the database. This can prevent them (or others using the same credentials) from logging in again if you enforce single-session-per-user.
- **Session Timeout:**
  - Implement a timeout (e.g., 30 minutes of inactivity) after which the session is automatically marked as "inactive" in the database. On each user action, update a `last_active` timestamp in the session record. When a user tries to log in, check for "active" sessions older than the timeout and mark them as "inactive" before allowing a new login.
- **Heartbeat/Ping:**
  - Use JavaScript in the frontend to periodically send a "still active" signal to the backend. If the backend stops receiving heartbeats, mark the session as "abandoned" or "inactive" after a grace period.
- **Force Login Option:**
  - When a user tries to log in and an active session exists, offer a "force login" button that will invalidate the previous session and allow the new login.
- **Session Cleanup Job:**
  - Run a scheduled job (e.g., cron or background task) to periodically scan for and clean up abandoned sessions.
- **Best Practice:**
  - Combine session timeout with a `last_active` field for robust handling. Always allow users to recover from abandoned sessions, either by waiting for timeout or by providing a "force login" option. These strategies ensure users are not locked out due to network issues or browser closes, and help maintain accurate session state in the database.
